...
function handleArray(schema: ISchemaRouteParam, pValue: ParamValue | ParamValue[], pName: string): ParamValueOutput[] {
    ...
    return pArray.map((value, index) => handlePropertie(itemSchema, value, `${pName}[${index}]`));
}

function handlePropertie(schema: ISchemaRouteParam, pValue: ParamValue, pName: string): ParamValueOutput | ParamValueOutput[] {
...
function objectValidator(schemaParams: ISchemaRouteParams, routeParams: IRouteParams): IRouteParamsOutput {
    ...
    const checkedParams: HandleParamsTuple[] = params.map(([pName, pValue]) => {
        ...
        const newValue = handlePropertie(schema, pValue, pName);
